...
import xbmc
import xbmcgui
from default import ADDON, ADDONID, LOCALIZE
from lib.class_xbmc import Widgets_Monitor, Widgets_Player
from lib.utils import log, media_path, media_streamdetails
...
class Main:
    ...
    def _fetch_tvshows_recommended(self, request: str):
        ...
        if not self.Monitor.abortRequested():
            ...
            p_json_query: dict = simplejson.loads(json_query)
            if 'result' in p_json_query and 'tvshows' in p_json_query['result']:
                self._clear_properties(request)
                count = 0
                for item in p_json_query['result']['tvshows']:
                    ...
                    json_query2 = xbmc.executeJSONRPC(
                        ...
                        '"limits": {"end": 1}}, '
                        '"id": 1}' % item['tvshowid'])
                    p_json_query2: dict = simplejson.loads(json_query2)
                    if ('result' in p_json_query2
                        and p_json_query2['result'] is not None
                            and 'episodes' in p_json_query2['result']):
                        for item2 in p_json_query2['result']['episodes']:
        ...
        if not self.Monitor.abortRequested():
            ...
            if request == 'RecentEpisode' and self.RECENTITEMS_UNPLAYED:
                ...
                json_query = xbmc.executeJSONRPC(
                    '%s "sort": {"method": "random" }}}' % json_string)
            p_json_query: dict = simplejson.loads(json_query)
            if 'result' in p_json_query and 'episodes' in p_json_query['result']:
                self._clear_properties(request)
                count = 0
                for item in p_json_query['result']['episodes']:
                    count += 1
                    # if count <= 2:
    ...
    def _fetch_seasonthumb(self, tvshowid, seasonnumber):
        ...
        p_json_query: dict = simplejson.loads(json_query)
        if 'result' in p_json_query and 'seasons' in p_json_query['result']:
            for item in p_json_query['result']['seasons']:
                season = f"{float(item['season']):.2f}"
        ...
        if not self.Monitor.abortRequested():
            ...
            if request == 'RecommendedMusicVideo':
                ...
                json_query = xbmc.executeJSONRPC(
                    '%s "sort": {"method": "random"}}}' % json_string)
            p_json_query: dict = simplejson.loads(json_query)
            if 'result' in p_json_query and 'musicvideos' in p_json_query['result']:
                self._clear_properties(request)
                count = 0
        ...
        if not self.Monitor.abortRequested():
            ...
            if request == 'RecommendedAlbum':
                ...
                json_query = xbmc.executeJSONRPC('%s "sort": '
                                                 ...
                                                 % json_string)
            p_json_query: dict = simplejson.loads(json_query)
            if 'result' in p_json_query and 'albums' in p_json_query['result']:
                self._clear_properties(request)
                count = 0
                for item in p_json_query['result']['albums']:
                    count += 1
                    # if count <= 2:
    ...
    def _fetch_artist(self, request):
        if not self.Monitor.abortRequested():
            ...
            json_query = xbmc.executeJSONRPC('{"jsonrpc": "2.0", '
                                             ...
                                             '"limits": {"end": %d}}, "id": 1}' % self.LIMIT)
            p_json_query: dict = simplejson.loads(json_query)
            if 'result' in p_json_query and 'artists' in p_json_query['result']:
                self._clear_properties(request)
                count = 0
                for item in p_json_query['result']['artists']:
                    count += 1
                    # if count <= 2:
        ...
        if not self.Monitor.abortRequested():
            ...
            if request == 'RandomSong' and self.RANDOMITEMS_UNPLAYED == "True":
                ...
                json_query = xbmc.executeJSONRPC(
                    '%s  "sort": {"method": "random"}}}' % json_string)
            p_json_query: dict = simplejson.loads(json_query)
            if 'result' in p_json_query and 'songs' in p_json_query['result']:
                self._clear_properties(request)
                count = 0
                for item in p_json_query['result']['songs']:
                    count += 1
                    # if count <= 2:
        ...
        if not self.Monitor.abortRequested():
            ...
            for content in ['audio', 'video', 'unknown']:
                json_query = xbmc.executeJSONRPC('{"jsonrpc": "2.0", '
                                                 ...
                                                 '"broken"]}, '
                                                 '"id": 1}')
                p_json_query: dict = simplejson.loads(json_query) if json_query else {}
                if 'result' in p_json_query and 'addons' in p_json_query['result']:
                    # find plugins and scripts
                    for item in p_json_query['result']['addons']:
            ...
            if p_json_query:
                self.WINDOW.setProperty(f"{request}.Count", str(
                    p_json_query['result']['limits']['total']))
            del p_json_query
    ...
    def _clear_properties(self, request: str):
        """Clears home window properties of the requested type
